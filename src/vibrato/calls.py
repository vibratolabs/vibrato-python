...
class Calls:
    ...
    def create_call_from_task_template(self, request: components.CreateFromTaskTemplate) -> operations.CreateCallFromTaskTemplateResponse:
        r"""Create a new call from a task template"""
        hook_ctx = HookContext(operation_id='createCallFromTaskTemplate', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        base_url = utils.template_url(*self.sdk_configuration.get_server_details())
        
        url = base_url + '/calls/create_from_task_template/'
        ...
        req_content_type, data, form = utils.serialize_request_body(request, components.CreateFromTaskTemplate, "request", False, False, 'json')
        if req_content_type is not None and req_content_type not in ('multipart/form-data', 'multipart/mixed'):
            headers['content-type'] = req_content_type
        if data is None and form is None:
            raise Exception('request body is required')
        ...
        res = operations.CreateCallFromTaskTemplateResponse(http_meta=components.HTTPMetadata(request=req, response=http_res))
